...
class ProfileDialog(QDialog):
    def __init__(self, profile_dir):
        ...
        self.load_profiles()
        self.new_btn = QPushButton("신규 설치장소 + 시스템 생성")
    ...
    def accept_selection(self):
        ...
        if current:
            ...
            QMessageBox.warning(self, "선택 오류", "기존 프로파일을 선택하거나 신규 생성을 하세요.")
...
class BatteryMonitorUI(QMainWindow):
    ...
    def save_site_info(self):
        self.settings.setValue("site", self.site_edit.text())
        self.settings.setValue("system", self.system_edit.text())
        self.settings.sync()
