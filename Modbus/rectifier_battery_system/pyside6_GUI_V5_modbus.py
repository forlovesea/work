...
class ModbusGUI(QWidget):
    def __init__(self):
        ...
        self.label_batt.setFixedSize(300, 800)
        layout.addWidget(self.label_batt, alignment=Qt.AlignLeft)
    ...
    def add_battery_overlay_buttons(self):
        ...
        module_height = label_h // 10
        for i in range(10):
            btn = QPushButton(self.label_batt)
            btn.setGeometry(0, label_h - (i + 1) * module_height, label_w, module_height)
            btn.setStyleSheet("background-color: rgba(0,0,0,0);")  # 투명
            btn.clicked.connect(lambda checked, n=i + 1: self.show_module_number(n))

    def show_module_number(self, module_number):
        QMessageBox.information(self, "Battery Module", f"🔋 Module {module_number} 선택됨")
    ...
    def update_buttons(self, enabled):
        for btn in [self.btn_vdc, self.btn_iload, self.btn_acf, self.btn_ibatt, self.btn_soc,
                    self.btn_read_alarms, self.btn_monitor]:
