...
class ModuleDetailDialog(QDialog):
    def __init__(self, module_no, parent=None):
        ...
        #print("===== ModuleDetailDialog DEBUG =====")
        #print(f"module_no : {module_no}")
        #print(f"equip_id  : {equip_id}")
        #print(f"module_map 전체 : {self.parent_ui.module_map}")

        module_data = self.parent_ui.module_data.get(equip_id)

        #print(f"module_data : {module_data}")
        #print(f"module_data 전체 : {self.parent_ui.module_data}")
        
        # 상세 데이터 확인
        #if module_data:
        #    print("volt :", module_data.get("volt"))
        #    print("status :", module_data.get("status"))
        #    print("soc :", module_data.get("soc"))
        #    print("soh :", module_data.get("soh"))
        #    print("cells :", module_data.get("cells"))
        #    print("temps :", module_data.get("temps"))
        #else:
        #    print("⚠ module_data 없음")
        ...
        label_barcode = QLabel(f"5.BAR: {barcode_txt}" if barcode_txt is not None else " BAR: -")
 ...
 #################################################################################   
    ...
    def create_fault_table(self):
        ...
        for row, fault in enumerate(faults):            
            values = [f"#{(row+1):02d}", fault["module"], fault["cell"], fault["volt"], fault["temp"]]
            for col, val in enumerate(values):
